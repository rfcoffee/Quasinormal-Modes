(* Calculate quasinormal modes of (SAdS-like) black holes in the Scalar-Tensor theory. For details, see: arxiv.org/abs/arXiv : 1709.01641 *)

(* We work with de-Sitter unit *)
seriesorder=70;
rhlist=Range[5,10,1];
numrh=Length[rhlist];
...
For[i=1,i<=numrh,i++,
  ...
  h[r_]:=(r^2+2)/(2*(r^2+1));
  F[r_]:=3/4+r^2/(12)-2*m/r+1/(4*r)*ArcTan[r];
  f[r_]:=F[r]/h[r];
  V[r_]:=f'[r]/r+c*h[r]/r^2;
  rh=rhlist[[i]];
  m=rh/2*(3/4+rh^2/12+1/(4*rh)*ArcTan[rh]);
  c=0;
...
Export["out_ScalarTensor.dat",Join[Partition[rhlist,1],Join[Partition[Thlist,1],sollist,2],2],"Table"];
Exit[];
